  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      try {
        ...
        // Fallback or override: Try the absolute http://localhost:8080 endpoint directly
        console.log("Relative API intake request failed/absent, targeting direct port 8080 URL:", e);
        usedEndpointUrl = "http://localhost:8080/api/intake/requests";
        response = await fetch(usedEndpointUrl);
      }
    ...
    if (link) {
      return (
        <a 
          ...
          className={`inline-flex items-center gap-1.5 px-2 py-1 rounded bg-blue-500/10 border border-blue-500/20 font-bold text-blue-400 hover:bg-blue-500/20 hover:text-blue-300 transition-all shadow-inner ${customClassName || 'text-xs'}`}
        >
          <span>{value}</span>
          <ExternalLink size={11} className="text-blue-400/70 shrink-0" />
        </a>
      );
    }
    return <span className={`text-slate-300 font-bold ${customClassName || 'text-xs'}`}>{value}</span>;
  };
