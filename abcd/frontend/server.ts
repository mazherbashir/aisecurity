...
import { CWE_SPECIFIC_PRESETS, GENERAL_SAST_PRESETS, SCA_JUSTIFICATION_PRESETS, normalizeCwe } from "./src/data/justificationPresets.js";
...
async function startServer() {
  ...
  // Mitigation & Approval Presets Catalog (server-side per CWE)
  let devMemCwePresets: Record<string, any[]> = { ...CWE_SPECIFIC_PRESETS };
  let devMemGeneralSastPresets: any[] = [...GENERAL_SAST_PRESETS];
  let devMemScaPresets: any[] = [...SCA_JUSTIFICATION_PRESETS];
  ...
  app.get("/api/heartbeat", async (req, res) => {
    ...
    try {
      ...
      // If the backend at 8080 is down/offline, we handle gracefully and return isServerOnline: false
      return res.json({ isServerOnline: false });
    }
  });
  ...
  function generateFallbackMitigationAssessment(prompt: string, type?: string, flawId?: string, flawSummary?: string): string {
  ...
  // AI API Route - supports both /api/ai and /api/ai/analyze
  const handleAiAnalyze = async (req: express.Request, res: express.Response) => {
    if (!useMocks) {
      try {
        ...
        console.log('[AI] Could not proxy to 127.0.0.1:8080, falling back to direct AI generation:', (error as Error).message);
      }
    }
    ...
    try {
      if (engine === 'azure') {
        ...
        // Fallback intelligent mitigation comment generator
