...
public class VeracodeService {
    ...
    public VeracodeReportDTO getFinalReport(String applicationName, String appId, String buildId,
        ...
        try {
            ...
            // Check missing SCA file paths for selected modules using deserialized object
            Set<String> scaFilePaths = new HashSet<>();
            if (report.getSca() != null && report.getSca().getVulnerableComponents() != null && report.getSca().getVulnerableComponents().getComponents() != null) {
                for (ScaComponent comp : report.getSca().getVulnerableComponents().getComponents()) {
                    if (comp.getFileName() != null) {
                        scaFilePaths.add(comp.getFileName().toLowerCase());
                    }
                    if (comp.getFilePathList() != null && comp.getFilePathList().getFilePaths() != null) {
                        for (ScaFilePath fp : comp.getFilePathList().getFilePaths()) {
                            if (fp.getValue() != null) {
                                scaFilePaths.add(fp.getValue().toLowerCase());
                            }
                        }
                    }
                }
            ...
            for (String moduleName : dto.selectedModules) {
                String mLower = moduleName.toLowerCase();
                if (mLower.endsWith(".zip") || mLower.contains("js files") || mLower.contains("javascript") || mLower.contains("python")) {
                    boolean hasScaRef = false;
                    String searchTarget = mLower;
                    if (searchTarget.contains(" within ")) {
                        searchTarget = searchTarget.substring(searchTarget.lastIndexOf(" within ") + 8).trim();
                    }
                    for (String scaPath : scaFilePaths) {
                        if (scaPath.contains(searchTarget)) {
                            hasScaRef = true;
                            break;
                        }
                        if (searchTarget.contains(scaPath) && !scaPath.trim().isEmpty()) {
                            hasScaRef = true;
                            break;
                        }
                    }
                    if (!hasScaRef && !moduleName.isEmpty()) {
                        dto.missingSCAForSelectedModules.add(moduleName);
                    }
                }
            }
            // Deduplicate missingSCAForSelectedModules
            dto.missingSCAForSelectedModules = dto.missingSCAForSelectedModules.stream().distinct().collect(Collectors.toList());
                ...
                for (PrescanModule m : prescan.getModules()) {
                    ...
                    boolean isAlreadySelected = dto.selectedModules.stream()
                            .anyMatch(selected -> {
                                ...
                                if (selectedInsideName && (modLower.startsWith("js files within ") || modLower.startsWith("javascript files within "))
                                         && (selLower.endsWith(".war") || selLower.endsWith(".jar") || selLower.endsWith(".ear"))) {
                                     selectedInsideName = false;
                                 }
